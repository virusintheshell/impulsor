...
namespace PRASYDE.ControlEscolar.WebAPI.Controllers
{
    ...
    public class ExpedienteDigitalController : ApiController
    {
        ...
        [Route("ObenerListaArchivos/{idAlumno}")]
        [HttpGet]
        public HttpResponseMessage ObtenerListado(string idAlumno)
        {
            HttpStatusCode Codigo_Respuesta = new HttpStatusCode();
            RespuestaGeneral Respuesta = new RespuestaGeneral();

            Respuesta = ExpedienteDigitalBusiness.ListadoArchivos(idAlumno, ref Codigo_Respuesta);
            return Request.CreateResponse(Codigo_Respuesta, Respuesta);
        }

        [Route("EditarArchivos")]
        [HttpPost]
        public HttpResponseMessage EditarArchivo()
        {
            ...
            else
            {
                if (HttpContext.Current.Request.Form.GetValues("Request") != null)
                {
                    ...
                    Respuesta = ExpedienteDigitalBusiness.Editar(request, ref Codigo_Respuesta);
                    return Request.CreateResponse(Codigo_Respuesta, Respuesta);
                }
                else
                {
